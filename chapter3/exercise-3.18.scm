  ...
  (lambda (val)
    (cases expval val
      (emptylist-val ()
                     '())
      (cons-val (first rest)
                (cons first (expval->list rest)))
      (else
       (error 'error-in-expval->list)))))
    ...
    (cases expression exp
      ...
      (unpack-exp (ids exp1 exp2)
                  (let ((vals (value-of exp1 env)))
                    (value-of exp2 (extend-env* ids (expval->list vals) env)))))))
...
(run&show "let* x = 30 in let* x = -(x, 1) y = -(x, 2) in -(x, y)")
(run&show "let u = 7 in unpack x y = cons(u, cons(3, emptylist)) in -(x,y)")
